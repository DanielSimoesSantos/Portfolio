...
splitter = RecursiveCharacterTextSplitter(
    chunk_size=2500,       # ~approx chars; tune after inspecting results
    chunk_overlap=300,     # modest overlap to capture boundary context
    separators=["\n\n", "\n", " ", ""],  # paragraph -> line -> word
)
...
def attach_metadata(chunks: List[str]) -> List[Dict]:
    ...
    for i, ch in enumerate(chunks):
        ...
        page_start = min(pages) if pages else "NA"
        page_end   = max(pages) if pages else "NA"
        out.append({
